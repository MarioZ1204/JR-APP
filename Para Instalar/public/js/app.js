...
const state = {
  ...
  lanUrls: [],
  tailscaleUrls: [],
  tailscaleAccount: 'jrburgerpasto@gmail.com',
  serverDates: null,
  moreNav: false,
  ticketOpen: false,
  loading: false,
...
async function boot() {
  try {
    ...
    state.tailscaleUrls = info.tailscale_urls || [];
    state.tailscaleAccount = info.tailscale_account || 'jrburgerpasto@gmail.com';
...
function lanAccessCard() {
  const lan = state.lanUrls || [];
  const ts = state.tailscaleUrls || [];
  const mail = esc(state.tailscaleAccount || 'jrburgerpasto@gmail.com');
  const alreadyRemote = !/^(localhost|127\.0\.0\.1)$/i.test(location.hostname);
  if (alreadyRemote) return '';
  if (!lan.length && !ts.length) {
    return `<p class="hint">El celular no entra con localhost. Use la IP WiFi del local (<b>http://IP:3000</b>) o Tailscale con la cuenta <b>${mail}</b>.</p>`;
  }
  return `
    <div class="lan-box">
      <div class="ticket-head">Acceso desde otro dispositivo</div>
      ${lan.length ? `
        <p class="hint"><b>WiFi / cable</b> — misma red del local (no datos móviles):</p>
        ${lan.map((u) => `<p class="lan-url">${esc(u)}</p>`).join('')}
      ` : ''}
      ${ts.length ? `
        <p class="hint"><b>Tailscale</b> — cuenta <b>${mail}</b> (PC y celular con la misma cuenta):</p>
        ${ts.map((u) => `<p class="lan-url">${esc(u)}</p>`).join('')}
      ` : `
        <p class="hint"><b>Tailscale</b> — en este PC y en el celular inicie sesión con <b>${mail}</b> (ejecute <b>conectar-tailscale.bat</b>). Luego reinicie JR Sistema para ver la dirección aquí.</p>
      `}
...
function tableCard(t) {
  ...
  const extra = t.order
    ? `${t.order.item_count} productos · ${money(orderAmt)}${t.order.combo ? ' · Combo' : ''}`
    : `${t.seats} personas`;
  return `
  ...
  return `
    <div class="pos">
      ...
      <aside class="pos-ticket${state.ticketOpen ? ' is-open' : ''}">
        ...
        <div class="pos-ticket-foot">
          ...
          ${(state.user?.role === 'admin' || state.user?.role === 'cashier') && active.length
            ? `<button type="button" class="btn warn block" data-act="go-bill">Cobrar</button>`
          ...
          ${billed || !canWaiterCancelOrder(o) ? '' : `<button type="button" class="btn danger block" data-act="cancel-order">${active.length ? 'Cancelar cuenta' : (o.takeaway || o.table_is_takeaway ? 'Cancelar pedido' : 'Liberar mesa')}</button>`}
        </div>
      </aside>
    </div>
    ...
    <div class="pos-dock${state.cartBump ? ' bump' : ''}">
      <button type="button" class="pos-dock-cart" data-act="toggle-ticket">
        <span>${active.length} prod.${o.combo ? ' · combo' : ''}</span>
    ...
    <div class="kds">
      ${filtered.map((o) => {
        ...
        <article class="card kds-card ${esc(o.status)} ${wait.minutes >= 10 ? 'late' : ''}">
          <div class="kds-top">
            <div>
              <h2>${esc(o.table_name)}${o.combo ? ' · Combo' : ''}</h2>
...
function billForm(o, openCash) {
  ...
  const canCharge = Boolean(openCash) && active.length > 0 && !['billed', 'cancelled'].includes(o.status);
  return `
    ...
    ${!active.length ? `
      <div class="alert warn">Este pedido no tiene productos. Agréguelos en la comanda o cancele el pedido.</div>
      <div class="row" style="gap:8px;margin-bottom:12px">
        <button type="button" class="btn primary" data-act="nav" data-view="comanda" data-id="${o.id}">Ir a la comanda</button>
        ${state.user?.role !== 'waiter' ? '<button type="button" class="btn danger" data-act="cancel-order-bill">Cancelar pedido</button>' : ''}
      </div>` : ''}
    ${!['billed', 'cancelled'].includes(o.status) && active.length ? `
    ...
    <div class="bill-grid">
      <div class="card">
        ...
        ${active.map((i) => `<div class="ticket-line"><span>${i.quantity}× ${esc(i.product_name)}${modsText(i) ? `<div class="notes">${esc(modsText(i))}</div>` : ''}</span><span class="line-amt">${money(i.quantity * i.unit_price)}</span></div>`).join('') || '<p class="hint">Sin productos</p>'}
      ...
      <form class="card" data-act="invoice" data-total="${total}" data-oid="${o.id}">
        ...
        <div class="field"><label>Descuento ($)</label>
          <input type="number" min="0" step="1" name="discount" data-act="bill-adj" data-field="discount" value="${discount}" ${promoOn || !canCharge ? 'disabled' : ''} />
          ${promoOn ? '<p class="hint">Segunda hamburguesa al 50%. No se puede editar a mano.</p>' : ''}</div>
        <div class="field"><label>Propina ($)</label>
          <input type="number" min="0" step="1" name="tip" data-act="bill-adj" data-field="tip" value="${tip}" ${!canCharge ? 'disabled' : ''} /></div>
        ...
        <div class="pay-tiles">
          ${Object.entries(PAY).map(([k, lab]) => `
            <label class="pay-tile pay-${k}">
              <span>${lab}</span>
              <input type="number" min="0" step="1" name="${k}" value="${k === 'efectivo' ? Math.round(total) : 0}" ${!canCharge ? 'disabled' : ''} />
            </label>`).join('')}
        </div>
        <button type="submit" class="btn primary block lg${state.busy === 'invoice' ? ' is-busy' : ''}" ${!canCharge || state.busy ? 'disabled' : ''}>${state.busy === 'invoice' ? 'Cobrando…' : 'Cobrar e imprimir recibo'}</button>
      </form>
    </div>`;
}
  ...
  if (section === 'cuenta') {
    return `
      ...
      <div class="card form-narrow" style="margin-top:14px">
        ...
        <p class="small muted">La app necesita el PC servidor encendido. En el local use WiFi; fuera del local use Tailscale en el PC y en el celular.</p>
      </div>`;
  }
...
async function onClick(e) {
  ...
  try {
    if (act === 'nav') {
      ...
      go(el.dataset.view, el.dataset.id || undefined);
      return;
    }
    ...
    if (act === 'cancel-order-bill') await cancelOrder({ backTo: 'facturar' });
    if (act === 'go-bill') {
      if (!state.order?.id) return;
      go('facturar', state.order.id);
      return;
    }
...
async function onTable(id) {
  ...
  if (t.is_takeaway) {
    if (state.view === 'facturar') {
      if (t.order) go('facturar', t.order.id);
      else toast('Ese para llevar no tiene pedido abierto', true);
      return;
    }
    if (t.order) {
      go('comanda', t.order.id);
      return;
    }
    if (t.status !== 'free') {
      toast('Ese para llevar quedó ocupado sin cuenta. Cancélelo o use Reiniciar salón.', true);
      return;
    }
    await openTakeaway();
    return;
...
function orderTableStatus(order) {
  const t = (state.tables || []).find((x) => x.id === order?.table_id);
  return t?.status || null;
}

function canWaiterCancelOrder(order) {
  if (state.user?.role !== 'waiter') return true;
  return orderTableStatus(order) !== 'waiting_payment';
}

async function cancelOrder({ backTo = 'mesas' } = {}) {
  if (!canWaiterCancelOrder(state.order)) {
    toast('No puede cancelar una cuenta por cobrar. Pida al cajero o al jefe.', true);
    return;
  }
  const empty = !(state.order?.items || []).some((i) => i.status !== 'cancelled');
  const takeaway = Boolean(state.order?.takeaway || state.order?.table_is_takeaway);
  const ok = await confirmDialog(
    empty
      ? (takeaway ? '¿Cancelar este pedido para llevar? No hay productos.' : '¿Liberar esta mesa? No hay productos en la cuenta.')
      : (takeaway ? '¿Cancelar este pedido para llevar? No se cobra nada.' : '¿Cancelar esta cuenta y dejar la mesa libre? No se cobra nada.'),
    { title: empty ? (takeaway ? 'Cancelar pedido' : 'Liberar mesa') : 'Cancelar cuenta', danger: true }
  );
  if (!ok) return;
  const reason = empty ? (takeaway ? 'Pedido para llevar cancelado' : 'Mesa liberada') : await promptDialog('¿Por qué se cancela?', { title: 'Motivo', placeholder: 'Opcional' });
  if (reason == null) return;
