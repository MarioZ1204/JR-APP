...
function mountApi(app) {
  ...
  app.get('/api/info', (_req, res) => {
    ...
    `).get();
    const access = accessUrls(Number(process.env.PORT || 3000));
    res.json({
      business_name: getSetting('business_name', 'JR Burger'),
      business_tagline: getSetting('business_tagline', ''),
      lan_urls: access.lan,
      tailscale_urls: access.tailscale,
      access_urls: access.all,
      tailscale_account: getSetting('tailscale_account', 'jrburgerpasto@gmail.com'),
  ...
  app.get('/api/me', requireAuth, (req, res) => {
    ...
    const setup = {
      ...
      needs_business: !s.business_name || s.business_name === 'Mi Restaurante',
  ...
  app.post('/api/orders/:id/cancel', requireAuth, requireRole('waiter', 'cashier'), (req, res) => {
    ...
    `).get(orderId);
    if (!order || ['billed', 'cancelled'].includes(order.status)) {
      return fail(res, 400, 'Esa cuenta ya está cerrada o cobrada');
    }
    if (req.user.role === 'waiter' && order.table_status === 'waiting_payment') {
      return fail(res, 403, 'No puede cancelar una cuenta por cobrar. Pida al cajero o al jefe.');
    }
    const closed = cancelOpenOrder(orderId, req.user.id, req.body.reason || 'Cuenta cancelada');
  ...
  app.get('/api/invoices', requireAuth, requireRole('cashier'), (req, res) => {
    const rows = db().prepare(`
      SELECT i.*,
        CASE WHEN COALESCE(o.takeaway, 0) = 1
          THEN ('Pedido ' || o.id || ' - Para llevar')
          ELSE t.name
        END AS table_name,
  ...
  app.get('/api/dashboard', requireAuth, requireRole(), (req, res) => {
    ...
    const recent = db().prepare(`
      SELECT i.id, i.number, i.total, i.created_at,
        CASE WHEN COALESCE(o.takeaway, 0) = 1
          THEN ('Pedido ' || o.id || ' - Para llevar')
          ELSE t.name
        END AS table_name
