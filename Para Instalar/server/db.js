...
function init() {
  ...
  seedCatalog(getDb());
  markCoreRecipeLines();
  const currentName = getSetting('business_name', '');
  if (!currentName || currentName === 'Mi Restaurante' || currentName === 'JR Restaurante') {
    setSetting('business_name', 'JR Burger');
  }
  if (!getSetting('tailscale_account')) {
    setSetting('tailscale_account', 'jrburgerpasto@gmail.com');
  }
  if (getSetting('business_name') === 'JR Burger' && getSetting('setup_completed', '0') !== '1') {
    /* JR Burger es el nombre por defecto válido; no fuerza wizard de nombre */
  }
  return db;
}
...
const DEFAULT_SETTINGS = {
  ...
  tailscale_account: 'jrburgerpasto@gmail.com',
