...
function takeawayDisplayName(orderId) {
  return `Pedido ${orderId} - Para llevar`;
}
...
function presentOrder(o) {
  ...
  return {
    ...
    table_name: takeaway ? takeawayDisplayName(o.id) : (o.table_name || o.table_name),
...
function tableList() {
  ...
  return tables.map((t) => {
    ...
    if (order) {
      ...
      if (full.takeaway) name = takeawayDisplayName(full.id);
    }
    return presentTable({ ...t, name, order: summary });
  });
}
