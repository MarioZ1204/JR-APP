# Empaqueta entrega en J:\Sistema\Sistema JR  y  J:\Sistema\Key
...
function Ensure-JDrive {
  if (Test-Path 'J:\') { return $true }
  $mirror = Join-Path $Src '_disco-J'
  New-Item -ItemType Directory -Path $mirror -Force | Out-Null
  $proc = Start-Process -FilePath 'subst' -ArgumentList "J:", $mirror -Wait -PassThru -NoNewWindow
  if ($proc.ExitCode -eq 0 -and (Test-Path 'J:\')) {
    Write-Host "Nota: J: no existia; se mapeo temporalmente a $mirror"
    return $true
  }
  return $false
}

if (-not (Ensure-JDrive)) {
  $JRoot = Join-Path $Src 'Sistema'
  Write-Host "AVISO: No se pudo usar J:. Se creo en: $JRoot"
} else {
  $JRoot = 'J:\Sistema'
}
...
$excludeFiles = @('producto.key', 'product.key', '.env')
